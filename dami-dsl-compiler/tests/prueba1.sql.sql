...
UPDATE public.alias SET authority_id = (
    SELECT authority_id FROM authority.id t WHERE t.autor_id = alias.id_autor);
...
INSERT INTO aux.work(work_id,id_obra,work_id,id_obra) 
...
INSERT INTO public.files(file_download_uri,file_name,file_type,autor_id,path,file_name,temporary,obra_id)
...
UPDATE public.authority_element SET authority = ( SELECT id FROM alias a JOIN aux.authority_element b ON a.autor_id,obra_id = b.autor_id,obra_id WHERE b.authority_element_id = authority_element.id);
    
UPDATE public.authority_element SET element = ( SELECT id FROM work a JOIN aux.authority_element b ON a.autor_id,obra_id = b.autor_id,obra_id WHERE b.authority_element_id = authority_element.id);
